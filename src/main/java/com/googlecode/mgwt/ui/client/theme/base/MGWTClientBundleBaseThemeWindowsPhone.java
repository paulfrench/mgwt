...
/**
 * The Windows Phone 8 client bundle
 * 
 */
public interface MGWTClientBundleBaseThemeWindowsPhone extends ClientBundle, MGWTClientBundle {

	@Source({ "css/groupinglist.css", "css/windowsphone/groupinglist.css" })
	GroupingList getGroupingList();

	@Source({ "css/progressbar.css", "css/windowsphone/progressbar.css" })
	...
	@Source({ "css/progressindicator.css", "css/windowsphone/progressindicator.css" })
	ProgressIndicatorCss getProgressIndicatorCss();

	@Source({ "css/header.css", "css/windowsphone/header.css" })
	HeaderCss getHeaderCss();

	@Source({ "css/slider.css", "css/windowsphone/slider.css" })
	SliderCss getSliderCss();

	@Source({ "css/carousel.css", "css/windowsphone/carousel.css" })
	CarouselCss getCarouselCss();

	@Source({ "css/list.css", "css/windowsphone/list.css" })
	ListCss getListCss();

	@Source("resources/list/arrow.png")
	DataResource listArrow();

	@Source({ "css/searchbox.css", "css/windowsphone/searchbox.css" })
	...
	@Source({ "css/buttons.css", "css/windowsphone/buttons.css" })
	ButtonCss getButtonCss();

	@Source({ "css/scrollpanel.css", "css/windowsphone/scrollpanel.css" })
	ScrollPanelCss getScrollPanelCss();

	@Source({ "css/buttonbar.css", "css/windowsphone/buttonbar.css" })
	ButtonBarCss getButtonBarCss();

	@Source({ "css/dialog.css", "css/windowsphone/dialog.css" })
	DialogCss getDialogCss();

	@Source({ "css/main.css", "css/windowsphone/main.css" })
	MainCss getMainCss();

	@Source({ "css/input.css", "css/windowsphone/input.css" })
	...
	@Source({ "css/pulltorefresh.css", "css/windowsphone/pulltorefresh.css" })
	PullToRefreshCss getPullToRefreshCss();

	@Source({ "css/tabbar.css", "css/windowsphone/tabbar.css" })
